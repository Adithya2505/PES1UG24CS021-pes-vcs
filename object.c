...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    // Step 1: Build the header string: "blob 16\0", "tree 42\0", etc.
    const char *type_str;
    ...
    // +1 to include the '\0' terminator that snprintf added

    // Step 2: Combine header + data into one buffer for hashing
    ...
    // Step 4: Check if already stored (deduplication)
    if (object_exists(id_out)) {
        free(full);
        return 0;
    }

    // Step 5: Build the shard directory path and create it
    char hex[HASH_HEX_SIZE + 1];
    ...
    // Step 7: fsync the temp file so data is on disk
    fsync(fd);
    close(fd);
    free(full);

    // Step 8: Rename temp file to final path (atomic on POSIX)
    char final_path[512];
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // Step 4: Parse the header — find the '\0' that ends it
    uint8_t *null_ptr = memchr(buf, '\0', file_size);
    if (!null_ptr) { free(buf); return -1; }

    // Step 5: Determine the type from the header string
